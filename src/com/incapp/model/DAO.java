...
public class DAO {
	...
	public DAO() throws Exception {
		...
		c=DriverManager.getConnection("jdbc:mysql://localhost:3306/onlineticketingdb","root","Suman@12");
	}
	public ArrayList<HashMap> getAdvocatesByCategory(String category) throws Exception {
		...
		while(rs.next()) {
			...
			advocate.put("depttime", rs.getString("depttime"));
			advocate.put("route", rs.getString("route"));

			advocate.put("totaltime", rs.getString("totaltime"));
	...
	public String registerAdvocate(HashMap advocate) throws Exception {
		PreparedStatement p=c.prepareStatement("insert into advocates (email,name,depttime,route,totaltime,photo,password,category,status) values (?,?,?,?,?,?,?,?,'active')");
		p.setString(1, (String)advocate.get("email"));
		p.setString(2, (String)advocate.get("name"));
		p.setString(3, (String)advocate.get("depttime"));
		p.setString(4, (String)advocate.get("route"));
		p.setString(5, (String)advocate.get("totaltime"));
		p.setBinaryStream(6, (InputStream) advocate.get("photo"));/*
